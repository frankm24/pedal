...
from pedal.assertions.feedbacks import (
    RuntimeAssertionFeedback,
    SandboxedValue,
    ExactValue,
...
from pedal.sandbox.result import (
    share_sandbox_context,
    is_sandbox_result,
    unwrap_value,
)
...
class assert_equal(RuntimeAssertionFeedback):
    ...
    justification_template = (
        "Left ({left}) and right ({right}) were not equal",
    ...
    def condition(self, left, right, exact_strings, delta):
        ...
        return errors(left, right) or not equality_test(
            left_value, right_value, exact_strings, delta
        )
...
class assert_in(RuntimeAssertionFeedback):
    ...
    def __init__(self, needle, haystack, **kwargs):
        super().__init__(
            SandboxedValue(needle), SandboxedValue(haystack), **kwargs
        )
...
class assert_contains_subset(RuntimeAssertionFeedback):
    ...
    def __init__(self, needles, haystack, **kwargs):
        super().__init__(
            SandboxedValue(needles), SandboxedValue(haystack), **kwargs
        )
...
class assert_is_dataclass(RuntimeAssertionFeedback):
    ...
    def __init__(self, value, **kwargs):
        super().__init__(
            SandboxedValue(value), ExactValue("a dataclass"), **kwargs
        )
...
class assert_true(RuntimeAssertionFeedback):
    ...
    def __init__(self, value, **kwargs):
        super().__init__(
            SandboxedValue(value), ExactValue("a true value"), **kwargs
        )
...
class assert_length_equal(RuntimeAssertionFeedback):
    ...
    def __init__(self, sequence, length, **kwargs):
        super().__init__(
            SandboxedValue(sequence), SandboxedValue(length), **kwargs
        )
...
class assert_length_greater(RuntimeAssertionFeedback):
    ...
    _expected_verb = (
        ...
        "to be greater than the length of",
    )
...
def value_to_pedal_type(value):
    if isinstance(unwrap_value(value), Exception):
        ...
        value_pedal_type = get_pedal_type_from_value(
            unwrap_value(value), evaluate
        )
...
class _compare_type(RuntimeAssertionFeedback):
    ...
    def __init__(self, value, expected_type, **kwargs):
        ...
        singular_name = share_sandbox_context(
            (
                value_pedal_type
                if isinstance(value_pedal_type, str)
                else value_pedal_type.singular_name
            ),
            value,
        )
        expected_pedal_type, expected_pedal_type_name = type_to_pedal_type(
            expected_type
        )
        ...
        super().__init__(
            SandboxedValue(singular_name),
            SandboxedValue(expected_pedal_type_name),
            **kwargs,
        )
...
class assert_output(RuntimePrintingAssertionFeedback):
    ...
    * Sorts the lines by default order
    ...
    def condition(self, execution, text, exact_strings):
        """Tests if the regex does not match the text"""
        return errors(execution) or not equality_test(
            self.get_output(execution),
            str(text.value),
            _exact_strings=exact_strings,
            _delta=None,
        )
...
class assert_output_contains(RuntimePrintingAssertionFeedback):
    ...
    def condition(self, execution, text, exact_strings):
        ...
        if not exact_strings:
            return (
                str(text.value).lower()
                not in self.get_output(execution).lower()
            )
...
class assert_output_regex(RuntimePrintingAssertionFeedback):
    ...
    def __init__(self, pattern, execution, exact_strings=False, **kwargs):
        super().__init__(
            execution, pattern, exact_strings=exact_strings, **kwargs
        )
    ...
    def condition(self, execution, text, exact_strings):
        ...
        return (
            errors(execution)
            or re.search(str(text.value), self.get_output(execution)) is None
        )
...
class assert_not_output_regex(RuntimePrintingAssertionFeedback):
    ...
    def condition(self, execution, text, exact_strings):
        """Tests if the regex does not match the text"""
        return (
            re.search(str(text.value), self.get_output(execution)) is not None
        )
...
class assert_has_variable(RuntimeAssertionFeedback):
    ...
    def __init__(self, sandbox, variable_name, **kwargs):
        super().__init__(
            SandboxedValue(sandbox), ExactValue(variable_name), **kwargs
        )
...
class assert_has_function(RuntimeAssertionFeedback):
    ...
    def __init__(self, sandbox, function_name, **kwargs):
        super().__init__(
            SandboxedValue(sandbox), ExactValue(function_name), **kwargs
        )
...
class ensure_coverage(AssertionFeedback):
    ...
    message_template = (
        "Your code coverage is not adequate. You must cover at "
        "least {at_least_message}% of your code to receive "
        "feedback. So far, you have only covered "
        "{coverage_message}%."
    )

    def __init__(self, at_least=0.5, **kwargs):
...
class ensure_called_uniquely(AssertionFeedback):
    ...
    title = "You Must Test Your Code"
    message_template = (
        "You have not tested the function {function_name} enough. "
    ...
    def __init__(
        self, function_name, at_least=1, ignore=None, why_ignored="", **kwargs
    ):
        report = kwargs.get("report", MAIN_REPORT)
        fields = kwargs.setdefault("fields", {})
        ...
        fields["instructor_ignored"] = instructor_ignored = len(
            unique_calls & ignore
        )
        if instructor_ignored:
            fields["instructor_ignore_message"] = (
                f" (but your instructor did not count {instructor_ignored} of the tests{why_ignored})"
            )
        else:
            fields["instructor_ignore_message"] = ""
